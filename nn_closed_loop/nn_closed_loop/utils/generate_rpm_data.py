...
def rpm_converter(system="DoubleIntegrator", model_name="default", model_type='torch'):
    ...
    bounded_model = BoundClosedLoopController.convert(
        torch_model, bound_opts={"zero-lb": True}, dynamics=dynamics.DoubleIntegrator()
    )
    ...
    for i in range(1, len(weights)):
        # import pdb; pdb.set_trace()
        top_augmented_row = np.hstack((weights[i], np.zeros((weights[i].shape[0], state_dim))))
        bottom_augmented_row = np.hstack((np.zeros((state_dim, weights[i].shape[1])), np.eye(state_dim)))
    ...
    for i in np.arange(-5, 5, 0.1):
        for j in np.arange(-5, 5, 0.1):
            ...
            if (np.abs(aug_pred.detach().numpy() - pred.detach().numpy()) > 5.53676e-04).any():
                print('ERROR!!!')
                print("Problem input: {}".format(test_input))
                raise ValueError
    ...
    X_mean, X_std = np.array([0,0]), np.array([np.sqrt((6+6)**2/12), np.sqrt((6+6)**2/12)])
    Y_mean, Y_std = np.array([4.75,0]), np.array([np.sqrt((5-4.5)**2/12), np.sqrt((0.25+0.25)**2/12)])

    layer_sizes = [layer.weights[1].shape[0] for layer in augmented_model.layers]
    ...
    shutil.move("weights.npz", "../../rpm2/Neural-Network-Reach/models/DoubleIntegrator/weights.npz")
    shutil.move("params.npz", "../../rpm2/Neural-Network-Reach/models/DoubleIntegrator/params.npz")
