...
def new():
    ### 2-step debugging
    backreachable_set = constraints.LpConstraint(range=np.array([[2., 2.625], [1.625, 2.25]]))
    target_sets = constraints.MultiTimestepLpConstraint(range=np.array([[[4.5, 5.], [-0.25, 0.25]], [[3.75, 4.75], [0.75, 1.25]]]))
    all_crown_matrices = [
        {'upper_A': tensor([[[-0.24625215,  0.11958137]]]), 'lower_A': tensor([[[0., 0.]]]), 'upper_sum_b': tensor([[-0.45236018]]), 'lower_sum_b': tensor([[-1.]])},
    ...
    for t in range(num_steps):
        ...
        lower_sum_b = deepcopy(crown_matrices['lower_sum_b'].detach().numpy()[0])
        upper_A = deepcopy(crown_matrices['upper_A'].detach().numpy()[0])
        upper_sum_b = deepcopy(crown_matrices['upper_sum_b'].detach().numpy()[0])
    ...
    for t in range(num_steps):
        ...
        print("self.dynamics.dynamics_step(xt[:, " + str(t) +"], ut[:, " + str(t) + "]) == xt[:, " + str(t+1) + "]")
        print('--')
