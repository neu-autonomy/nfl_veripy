...
# from jax_verify.src.types import Nest, SpecFn, Tensor  # pylint: disable=g-multiple-import
from matplotlib.patches import Rectangle
import matplotlib.pyplot as plt
from nn_closed_loop import dynamics as nfl_dynamics
from nn_closed_loop.utils.nn_jax import predict_future_states
from nn_closed_loop.utils.nn_jax import predict_mlp
from nn_closed_loop.utils.nn_jax import predict_next_state
from nn_closed_loop.utils.nn_jax import setup_nn
from nn_closed_loop.utils.utils import colors
import numpy as np
...
class LinFunExtractionConcretizer(concretization.BackwardConcretizer):
  ...
  Given an objective over an output, extract the corresponding linear function
  over a target node.
  ...
  def concrete_bound(self, graph, inputs, env, node_ref):
    ...
    target_linfun, _ = graph.backward_propagation(
        self._base_transform, env, {node_ref: initial_lin_expression},
        [self._target_index])
...
def backward_crown_bound_propagation_linfun(
  ...
  # We are first going to obtain intermediate bounds for all the activation of
  # the network, so that the backward propagation of the extraction can be done.
  crown_algorithm = concretization.BackwardConcretizingAlgorithm(
      backward_crown.backward_crown_concretizer)
  # BoundRetrieverAlgorithm wraps an existing algorithm and captures all of the
  # intermediate bound it generates.
  bound_retriever_algorithm = bound_utils.BoundRetrieverAlgorithm(
      crown_algorithm)
  bound_propagation.bound_propagation(
      bound_retriever_algorithm, function, *bounds,
  ...
  # Let's define what node we are interested in capturing linear functions for.
  ...
  # linear function of target index".
  # Note: If there is a need to handle a network with multiple output, it should
  # be easy to extend by making obj here a dict mapping output node to objective
  # on that output node.
  ...
  lin_fun_extractor_algorithm = fwd_bwd_alg(fwd_bound_applier,
                                            extracting_concretizer)
  # We get one target_linfuns per output.
  target_linfuns, _ = bound_propagation.bound_propagation(
      lin_fun_extractor_algorithm, function, *bounds,
      graph_simplifier=default_simplifier_without_linear)
...
def get_multi_step_reachable_sets_unrolled(
  ...
  """Compute all N-step reachable sets, by using dynamics(control(...(dynamics(control(init_state))...)) as function to verify."""
  logits_fn = functools.partial(predict_future_states, params, dynamics,
                                num_steps)
...
def get_multi_step_reachable_set(
  ...
  return get_multi_step_reachable_sets(params, xt_bounds, dynamics, num_steps,
                                       verif_fn)[-1]
...
def get_multi_step_reachable_sets(
    ...
    num_steps: int,
    verif_fn=jax_verify.backward_crown_bound_propagation):
  """Compute all N-step reachable sets and return them as list, by appending dynamics to end of NN."""
  bounds = [xt_bounds]
...
def get_one_step_reachable_set(
    ...
    dynamics: nfl_dynamics.Dynamics,
    verif_fn=jax_verify.backward_crown_bound_propagation):
  """Computes min/max that could be achieved at next timestep, subject to NN relaxation, prev state bounds, and dynamics, by appending dynamics to end of NN.
  ...
  Args:
    ...
    dynamics: nn_closed_loop.dynamics.Dynamics instance, describing state
...
def example_get_one_step_reachable_set():
  """Runs get_one_step_reachable_set on a simple NN, DoubleIntegrator, initial state set.

  Returns:
    output_bounds: jax_verify.IntervalBound rectangle of possible states at next
    timestep
  """
  dynamics = nfl_dynamics.DoubleIntegrator()
  initial_state_nominal = jnp.array([[0.8, 0.4]])
  input_bounds = jax_verify.IntervalBound(initial_state_nominal - 1.,
                                          initial_state_nominal + 1.)
  params, _, _ = setup_nn()
...
def check_closed_loop_bounds_are_valid(
    ...
    verif_fn=jax_verify.backward_crown_bound_propagation):
  """Confirms that bounds on next state (computed by ReachLP-Jax) are indeed outer bounds on next states via MC sampling.

  Also plots the given xt_bounds rectangle, the computed xt1_bounds rectangle,
  and the MC samples of x_{t} and their corresponding states at the next
  timestep, x_{t+1}.
  ...
  Args:
    ...
    dynamics: nn_closed_loop.dynamics.Dynamics instance, describing state
      transition dynamics of system being controlled by NN policy
    xt_bounds: jax_verify.IntervalBound describing rectangular set of possible
      states x_t at current timestep
    ...
    verif_fn: jax_verify propagation function to get output bounds on a fn (used
      by reach_fn)

  Returns:
    True if computed output_bounds are indeed outside of all the MC sampled pts,
    otherwise False
  """
  key = jax.random.PRNGKey(seed)
  num_states = xt_bounds.lower.shape[1]
  xt_mc = jax.random.uniform(
      key, (num_samples, num_states),
...
def train(params_init,
          ...
          plot=True):
  """Trains NN policy based on MPC imitation and safety (e.g., reachable set sizes).
  ...
  Args:
    ...
    dynamics: nn_closed_loop.dynamics.Dynamics instance, describing state
      transition dynamics of system being controlled by NN policy
    loss_weight: coefficient for linear combination of MPC and ReachLP losses.
      0=MPC, 1=ReachLP, should be btwn [0,1].
    num_epochs: number of epochs to train for
    loss_freq: number of epochs between loss calculations for plotting (can slow
      down training)
  ...
  def loss_multi_step_reachable_set_volume(params, input_bounds, dynamics):
    reachable_set = get_multi_step_reachable_set(params, input_bounds, dynamics,
                                                 num_steps)
  ...
  def loss_multi_step_reachable_set_origin_radial(params, input_bounds,
                                                  dynamics):
    """Compute loss as 'area' of rectangle (each point has area z=x^2+y^2)."""

    reachable_set = get_multi_step_reachable_set(params, input_bounds,
                                                 dynamics, num_steps)
    x0, y0 = reachable_set.lower[0]
    x1, y1 = reachable_set.upper[0]
    # Double integral of (x**2 + y**2) from x=[x0, x1] and y=[y0, y1]
    loss = (x1**3 - x0**3)*(y1-y0) + (y1**3 - y0**3)*(x1-x0)
    return loss
  def loss_multi_step_reachable_set_fwd_bwd_similar(params, input_bounds,
                                                    dynamics):
    ...
    area_bwd = jnp.product(reachable_sets_bwd[-1].upper -
                           reachable_sets_bwd[-1].lower)
    area_fwd = jnp.product(reachable_sets_fwd[-1].upper -
                           reachable_sets_fwd[-1].lower)
  ...
  def loss(params, input_bounds, dynamics, xs, us, loss_weight):
    """Linear combination of MPC loss and Reachable Set loss (loss_weight: 0 <- MPC --------- Reach -> 1)."""
    ...
    l = (loss_weight) * l_reach + (1 - loss_weight) * l_mpc
    return l

  @jax.jit
  def update(params, xs, us):
    grads = jax.grad(loss)(params, xt0_bounds, dynamics, xs, us,
                           loss_weight)
  ...
  for i in tqdm(range(num_epochs)):
    if i % loss_freq == 0:
      l = loss(params, xt0_bounds, dynamics, xs, us,
               loss_weight)
      losses[i // loss_freq, 0] = i
      losses[i // loss_freq, 1] = l
...
def get_next_action_with_mpc(x0s,
                             dynamics,
                             state_cost_matrix,
                             control_cost_matrix,
                             terminal_cost_matrix,
                             n_mpc=10):
  """Starting from an initial state, compute an optimal trajectory with MPC and return the first action.

  Args:
    x0s: np array of initial states
    dynamics: nn_closed_loop.dynamics.Dynamics instance, describing state
      transition dynamics of system being controlled by NN policy
    state_cost_matrix: LQR cost matrix to encourage state error to be minimized
      in optimal trajectory
    control_cost_matrix: LQR cost matrix to encourage usage of control input to
      be minimized in optimal trajectory
    terminal_cost_matrix: LQR terminal cost matrix (from solving discrete ARE)
  ...
  while step < n_mpc:
    ...
    constr = x[step + 1, :] == dynamics.dynamics_step(x[step, :], u[step, :])
...
def generate_mpc_dataset(
  ...
  """Computes a bunch of (state, action) pairs with an MPC algorithm starting in some initial state set.

  Args:
    dynamics: nn_closed_loop.dynamics.Dynamics instance, describing state
      transition dynamics of system being controlled by NN policy
    seed: jax random seed for initial state sampling
    num_initial_pts: number of states to randomly sample (and start from) in MPC
    xt0_bounds: jax_verify.IntervalBound with min/max values of initial state
      set
    num_timesteps: number of discrete timesteps to run MPC forward

  Returns:
    xs: jnp array of size (num_timesteps, num_initial_pts, num_states) with the
    states visited during the MPC rollouts
    us: jnp array of size (num_timesteps, num_initial_pts, num_control_inputs)
    with the actions taken during the corresponding state in xs
  """
  key = jax.random.PRNGKey(seed)
  xt_mc = jax.random.uniform(
      key, (num_initial_pts, dynamics.num_states),
  ...
  terminal_cost_matrix = solve_discrete_are(dynamics.At,
                                            dynamics.bt,
                                            state_cost_matrix,
                                            control_cost_matrix)
...
def run_sims(xt_mc, policy, dynamics: nfl_dynamics.Dynamics,
             num_timesteps: int = 5):
  """Runs simulations starting from states xt_mc, using given policy and dynamics."""
...
def run_policies(policies_dict,
                 dynamics,
                 xt0_bounds,
                 num_timesteps=3,
                 num_initial_pts=100,
                 seed=1701):
  """Simulates and plots trajectories with reachable sets using various NN controller realizations."""
  ...
  for d in settings:
    if d['params_key'] not in policies_dict: continue
    params = policies_dict[d['params_key']]
    d['params'] = params
    d['nn_function'] = functools.partial(batched_predict, params,
                                         dynamics.u_limits)

  for i in range(len(settings)):
    if settings[i]['params_key'] not in policies_dict: continue
    ...
    for t in range(num_timesteps):
      ...
      plt.scatter(
          xs[t, :, 0], xs[t, :, 1], marker=marker, color=colors(t), label=label)
