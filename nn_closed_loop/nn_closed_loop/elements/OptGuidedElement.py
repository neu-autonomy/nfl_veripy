...
import torch
import time
from nn_closed_loop.elements import Element, GuidedElement
...
class OptGuidedElement(GuidedElement):
    ...
    def split(self, target_set, dynamics, problems, full_samples=None, br_set_range=None, nstep=False, time_info=None, min_split_volume=0.00001):
        max_prob = problems[0]
        min_prob = problems[1]
        params = problems[2]
        ...
        if len(self.samples) == 0: # No samples in element -> bisect it hamburger style 
            ...
            buffer = 0 # Set buffer to zero if we're cutting out the target set
        ...
        elements = OptGuidedElement(new_ranges[0], self.policy, samples=split_samples[0], A_edge=new_A_edges[0]), OptGuidedElement(new_ranges[1], self.policy, samples=split_samples[1], A_edge=new_A_edges[1])
        ...
        for el in elements: # For each of the newly generated elements, we need to see if the BP constriants can be satisfied.
            ...
            if not is_terminal_cell:
                ...
                if idx[0] == 0: # If we're below the sample center, find min value and reverse the stored direction
                    prob = min_prob
                    el.A_t_br = -el.A_t_br
                ...
                if 'sample_limits' in locals():
                    ...
                    for i in range(dynamics.At.shape[0]):
                        ...
                        if el.ranges[i, 0] < sample_limits.T[i, 0]:
                            dist_min = np.abs(el.ranges[i, 0] - sample_limits.T[i, 0])
                        if el.ranges[i, 1] > sample_limits.T[i, 1]:
                            dist_max = np.abs(el.ranges[i, 1] - sample_limits.T[i, 1])
                    ...
                    if dynamics.At.shape[0] == 6:
                        dist = np.linalg.norm(np.max(np.abs(el.ranges.T-sample_center), axis=0)[0:3], 1)
                    else:
                        dist = np.linalg.norm(np.max(np.abs(el.ranges.T-sample_center), axis=0), 1)
                el.prop = dist
                
                volume = el.volume
                br_set_volume = np.prod(br_set_range[:,1] - br_set_range[:,0], axis=0)
