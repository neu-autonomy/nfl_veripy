...
class AdaptiveSimGuidedPartitioner(Partitioner):
    def __init__(self, N=20, k_NN =1, tolerance_eps=0.01, tolerance_expanding_step=0.001,tolerance_range=0.05):
        Partitioner.__init__(self)
        self.N= N
        self.tolerance_eps= tolerance_eps
   ...
   # def grab_from_M(self, M, output_range_sim):
    ...
    #tolerance_range=0.05
        ...
        output_range_sim = np.empty(sampled_output_.shape[1:]+(2,))
        output_range_sim[:,0] = np.min(sampled_output_, axis=0)
        output_range_sim[:,1] = np.max(sampled_output_, axis=0)
        ...
        center_NN=kdt.query(sampled_output_center.reshape(1,-1), k=self.k_NN, return_distance=False)
        ...
        while terminating_condition==False:
            ...
            np.all((output_range_sim[:,1] - output_range_new[:,1]) >= 0):
                ...
                tol = 5
                if np.max(abs(delta_step))<self.tolerance_step:

                    diff=(input_range-input_range_new)

                    max_range= (np.max(abs(diff)))
                    if max_range<self.tolerance_range:# or abs(max_range-prev_range)<tol:
                        ...
                        if idx<0:
                            ...
                            while np.max(abs(delta_step))<self.tolerance_step:
                                idx-=1
                                if idx<0:
                                    break
                                else:
